...
## What is React 
웹사이트의 개발은 사용자 컴퓨터의 브라우저에서 돌아가는 프론트엔드와 서버에서 돌아가는 백엔드로 나눌 수 있습니다.

<b>리액트</b>는 자바스크립트 라이브러리의 하나로서 사용자 인터페이스를 만들기 위해 사용됩니다. 페이스북과 개별 개발자 및 기업들 공동체에 의해 유지보수되고 있습니다.   
리액트는 Single Page Apps (SPA)를 쉽게 만들 수 있으며 UI 컴포넌트를 구축하기 위한 도구입니다.  

<b>등장이유 </b> : 기존의  MVC 로 만들어진 웹사이트에서 간단한 내용을 업데이트하기 위해 그 페이지 전체를 새로 로딩하는 것은 자원의 낭비일 수 있습니다. (내용을 새로 고치기 위해서는 웹페이지를 다시 로딩할 필요가 있습니다 )
이때 클라이언트 화면에 새로 고친 내용을 보여주기 위해 서버와 데이터베이스가 해당 게시물의 바뀐내용과 그 앞뒤의 내용과 해당 아이디와 댓글과 그 페이지의 다른 내용등을 모두 읽고 그 내용을 클라이언트의 브라우저로 보냅니다. 그리고 이러한 과정은 비록 <b>아주 적은 내용이 바뀌었을지라도 매번 실행되고</b> 그렇게 된다면 자원이 낭비된다고 볼 수 있습니다. 

>MVC (모델-뷰-컨트롤러) 는 사용자 인터페이스, 데이터 및 논리 제어를 구현하는데 널리 사용되는 소프트웨어 디자인 패턴입니다. 소프트웨어의 비즈니스 로직과 화면을 구분하는데 중점을 두고 있습니다. 좀더 자세한 내용은 , https://developer.mozilla.org/ko/docs/Glossary/MVC 보세요

React의  최초 출시일은 2013년 5월 29일 이며, 웹플랫폼, 자바스크립트언어를 사용합니다.   


모든 코딩을 마친 후에 이것을 배포할 형태로 만들어서 즉 빌드를 하며 브라우저가 이해할 수 있는  자바스크립트로 변환된다. 이것을 HTML, CSS 파일들과 패키징해서 서버에 올려놓으면 브라우저가 이 패키징한 것을 받아가서 정해진 대로 데이터를 요청하고 랜더링해서  클라이언트의 브라우저에 보여줍니다 
...
**모델** 은 어플리케이션에서 사용하는 데이터를 관리하는 영역이고    
**뷰** 는 사용자에게 보이는 부분입니다.   
